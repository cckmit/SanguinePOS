...
public class clsWaiterWiseItemReport 
{
    DecimalFormat gDecimalFormat = clsGlobalVarClass.funGetGlobalDecimalFormatter(); 
    public void funWaiterWiseItemReport(String reportType, HashMap hm,String dayEnd)
    {
        try
        {
            InputStream is = this.getClass().getClassLoader().getResourceAsStream("com/POSReport/reports/rptWaiterWiseItemReport.jasper");
            ...
            if(reportType.equalsIgnoreCase("Excel Report"))
            {
                ...
                for (clsBillDtl obj : listOfWaiterWiseItemSales)
                {
                    List<String> arrListItem = new ArrayList<String>();
                    arrListItem.add(obj.getStrWShortName());
                    arrListItem.add(obj.getStrItemName());
                    arrListItem.add(obj.getDteBillDate());
                    arrListItem.add(String.valueOf(decFormat.format(obj.getDblQuantity())));
                    arrListItem.add(String.valueOf(gDecimalFormat.format(obj.getDblRate())));
                    arrListItem.add(String.valueOf(gDecimalFormat.format(obj.getDblAmount())));
                    totalQty = totalQty + obj.getDblQuantity();
                    totalAmt = totalAmt + obj.getDblAmount();
                    mapExcelItemDtl.put(i, arrListItem);
                    i++;
                }
                arrListTotal.add(String.valueOf(decFormat.format(totalQty)) + "#" + "4");
                arrListTotal.add(String.valueOf(gDecimalFormat.format(totalAmt)) + "#" + "6");
                ...
                arrparameterList.add("Waiter" + " : " + waiterCode);
                ...
                funCreateExcelSheet(arrparameterList, arrHeaderList, mapExcelItemDtl, arrListTotal, "waiterWiseExcelSheet",dayEnd);
            }
        }
        catch (Exception e)
        {
