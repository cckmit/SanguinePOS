 ...
 * @date Aug 26, 2017
 */
public class clsTextFormat19ForBill implements clsBillGenerationFormat
{

    private final DecimalFormat decimalFormat = new DecimalFormat("#.###");
    private final SimpleDateFormat ddMMyyyyAMPMDateFormat = new SimpleDateFormat("dd-MM-yyyy hh:mm a ");
    private final clsUtility objUtility = new clsUtility();
    private final clsUtility2 objUtility2 = new clsUtility2();
    private final clsPrintingUtility objPrintingUtility = new clsPrintingUtility();
    private final DecimalFormat stdDecimalFormat = new DecimalFormat("######.##");
    private final DecimalFormat decimalFormatFor2DecPoint = new DecimalFormat("0.00");
    private final DecimalFormat decimalFormatFor3DecPoint = new DecimalFormat("0.000");
        ...
        try
        {
            ...
            else
            {
                if ("sales report".equalsIgnoreCase(formName))
                {
                    ...
                    long dateDiff = objUtility.funCompareDate(billDate, objUtility.funGetPOSDateForTransaction());
                    if (dateDiff > 0)
                    {
                        billhd = "tblqbillhd";
                        billdtl = "tblqbilldtl";
            ...
            String sqlOrderNo = "select a.strBillNo,a.dteBillDate,a.strPOSCode,a.intOrderNo  "
                    + "from " + billhd + " a "
                    + "where a.strBillNo=? "
                    + "and a.strPOSCode=? "
                    + "and date(a.dteBillDate)=? ";
            PreparedStatement psOrderNo = clsGlobalVarClass.conPrepareStatement.prepareStatement(sqlOrderNo);
            psOrderNo.setString(1, billNo);
            psOrderNo.setString(2, posCode);
            psOrderNo.setString(3, billDate);
            ResultSet rsOrderNo = psOrderNo.executeQuery();
            if (rsOrderNo.next())
            {
                String orderNo = rsOrderNo.getString(4);
                BillOut.write("  Your order no is " + orderNo);
                BillOut.newLine();
                BillOut.newLine();
                BillOut.newLine();
                BillOut.newLine();
