 ...
 * @date Aug 26, 2017
 */
public class clsTextFormat2ForBill implements clsBillGenerationFormat
{
    ...
    public void funGenerateBill(String billNo, String reprint, String formName, String transType, String billDate, String posCode, String viewORprint)
    {
        ...
        try
        {
            ...
            String SQL_BillDtl = "select SUM(dblQuantity),strItemName,SUM(dblAmount),strItemCode from " + billdtl + " where strBillNo=? and tdhYN='N' and date(dteBillDate)=? group by strItemCode ;";
            ...
            while (rs_BillDtl.next())
            {
                ...
                //objPrintingUtility.funWriteToText(BillOut, rs_BillDtl.getString(1), rs_BillDtl.getString(2).toUpperCase(), rs_BillDtl.getString(3),"Format2");
                objPrintingUtility.funWriteToText(BillOut, rs_BillDtl.getString(1), rs_BillDtl.getString(2).toUpperCase(), String.valueOf(amount), "Format2");
                BillOut.newLine();
                ...
                if (cntRecord > 0)
                {
                    ...
                    {
                        objPrintingUtility.funWriteToText(BillOut, "", rs_modifierRecord.getString(1).toUpperCase(), rs_modifierRecord.getString(3), "Format2");
                        BillOut.newLine();
                    }
            ...
            if (clsGlobalVarClass.gPointsOnBillPrint)
            {
                ...
                if (rsCRMPoints.next())
                {
                    objPrintingUtility.funWriteTotal("POINTS ", rsCRMPoints.getString(1), BillOut, "Format2");
                }
                rsCRMPoints.close();
                BillOut.newLine();
            }
            ...
            {
                objPrintingUtility.funWriteTotal("DISCOUNT  " + discPer + "%", discount, BillOut, "Format1");
                BillOut.newLine();
            }
            /*
             * if (tax.trim().length() > 0 && !"0.00".equalsIgnoreCase(tax)) {
             * objPrintingUtility.funWriteTotal("TAX", tax, BillOut); BillOut.newLine(); }
             */
            while (rsTax.next())
            {
