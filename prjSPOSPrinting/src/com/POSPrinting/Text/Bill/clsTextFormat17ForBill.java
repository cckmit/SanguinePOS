 ...
 * @date Aug 26, 2017
 */
public class clsTextFormat17ForBill implements clsBillGenerationFormat
{
	...
	try
	{
		...
		else
		{
		    ...
		    BillOut.write(Linefor5);
		    BillOut.newLine();
		    objPrintingUtility.funWriteTotal(printGroupName + " TOTAL", gDecimalFormat.format(groupTotal), BillOut, "Format5");
		    BillOut.newLine();
		    BillOut.write(Linefor5);
		...
		if (saleQty > 0)
		{
		    ...
		    {
			objUtility2.funPrintContentWithSpace("Right", gDecimalFormat.format(rs_BillDtl.getDouble(4)), 9, BillOut);//Amount
		    }
		    BillOut.newLine();
			...
			while (rs_modifierRecord.next())
			{
			    ...
			    {
				objUtility2.funWriteToTextformat5(BillOut, "", rs_modifierRecord.getString(1).toUpperCase(), gDecimalFormat.format(rs_modifierRecord.getDouble(3)), "Format5");
				BillOut.newLine();
			    }
	    ...
	    double advAmt = Double.parseDouble(advAmount);
	    if (delCharges > 0)
	    {
		objPrintingUtility.funWriteTotal("DELV. CHARGE", gDecimalFormat.format(delCharges), BillOut, "Format5");
		BillOut.newLine();
	    }
	    if (advAmt > 0)
	    {
		objPrintingUtility.funWriteTotal("ADVANCE", gDecimalFormat.format(advAmt), BillOut, "Format5");
		BillOut.newLine();
	    }
	    BillOut.write(Linefor5);
	    BillOut.newLine();
		...
		if (rsPrintGTYN.next())
		{
			...
			if (dtlBillSeriesBillNo.length > 0)
			{
				...
				{
				    BillOut.newLine();
				    objPrintingUtility.funWriteTotal(dtlBillSeriesBillNo[i] + " TOTAL(ROUNDED)", gDecimalFormat.format(rsPrintGT.getDouble(2)), BillOut, "Format5");
	    ...
	    {
		BillOut.newLine();
		objPrintingUtility.funWriteTotal("TIP AMT", gDecimalFormat.format(rs_BillHD.getDouble(29)), BillOut, "Format5");
		BillOut.newLine();
	    }
