 ...
 * @author sanguine_Vin
 */
public class clsTextFormat22ForBill implements clsBillGenerationFormat
{
	...
	try
	{
	   ...
	   // advAmount = rs_BillHD.getString(14);
		...
		BillOut.write(Linefor5);
		BillOut.newLine();
		BillOut.write("  QTY ITEM NAME           RATE    AMT");
		BillOut.newLine();
		BillOut.write(Linefor5);
		BillOut.newLine();
		
		String SQL_BillDtl = "select sum(a.dblQuantity),left(a.strItemName,17) as ItemLine1"
		    + " ,MID(a.strItemName,17,LENGTH(a.strItemName)) as ItemLine2"
		...
		while (rs_BillDtl.next())
		{
			...
			if (Double.parseDouble(qty) > 0)
			{
			    ...
			    objPrintingUtility.funPrintContentWithSpace("Right", qty, 5, BillOut);//Qty Print
			    BillOut.write(" ");
			    objPrintingUtility.funPrintContentWithSpace("Left", rs_BillDtl.getString(2), 17, BillOut);//Item Name
			    objPrintingUtility.funPrintContentWithSpace("Right", gDecimalFormat.format(rs_BillDtl.getDouble(7)),8, BillOut);
			    if (flgComplimentaryBill)
			    {
				objPrintingUtility.funPrintContentWithSpace("Right", "0.00", 8, BillOut);//Amount
			    }
			    else
			    {
				objPrintingUtility.funPrintContentWithSpace("Right", gDecimalFormat.format(rs_BillDtl.getDouble(4)), 8, BillOut);//Amount
			    }
			    BillOut.newLine();
				...
				if (line.length() > 17)
				{
				    BillOut.write("      " + line.substring(0, 17));
				    BillOut.newLine();

				    BillOut.write("      " + line.substring(17, line.length()));
				    BillOut.newLine();
				}
				else
				{
				...
				while (rs_modifierRecord.next())
				{
				    ...
				    if (modifierAmt > 0)
				    {
					objPrintingUtility.funPrintContentWithSpace("Right", String.valueOf(modifierQty), 5, BillOut);//Qty Print
					BillOut.write(" ");
				    }
				    else
				    {
					objPrintingUtility.funPrintContentWithSpace("Right", "", 5, BillOut);//Qty Print
					BillOut.write(" ");
				    }
				    objPrintingUtility.funPrintContentWithSpace("Left", rs_modifierRecord.getString(1).toUpperCase(), 19, BillOut);//Item Name
				    if (flgComplimentaryBill)
				    {
					objPrintingUtility.funPrintContentWithSpace("Right", "0.00", 7, BillOut);//Amount
				    }
				    else
				    {
					objPrintingUtility.funPrintContentWithSpace("Right", gDecimalFormat.format(rs_modifierRecord.getDouble(3)), 7, BillOut);//Amount
				    }
				    BillOut.newLine();
				}
