 ...
 * @date Aug 26, 2017
 */
public class clsTextFormat6ForBill implements clsBillGenerationFormat
{
    ...
    public void funGenerateBill(String billNo, String reprint, String formName, String transType, String billDate, String posCode, String viewORprint)
    {
        try
        {
            ...
            while (rs_BillDtl.next())
            {
                ...
                BillOut.newLine();
                objPrintingUtility.funPrintItemDetails(rs_BillDtl.getString(5), rs_BillDtl.getString(1), rs_BillDtl.getString(6), rs_BillDtl.getString(3), BillOut, "Format6", flag_DirectBillerBlill);
                ...
                if (cntRecord > 0)
                {
                    ...
                    SQL_ModifierDtl = "select strModifierName,dblQuantity,dblAmount "
                            ...
                            + " and strModifierCode='M99' and date(dteBillDate)=? ";
                    if (!clsGlobalVarClass.gPrintZeroAmtModifierOnBill)
                    {
            ...
            if (clsGlobalVarClass.gPointsOnBillPrint)
            {
                ...
                if (rsCRMPoints.next())
                {
                    objPrintingUtility.funWriteTotal("POINTS ", rsCRMPoints.getString(1), BillOut, "Format6");
                }
                rsCRMPoints.close();
                BillOut.newLine();
            }
            ...
            objPrintingUtility.funWriteTotal("SUB TOTAL", subTotal, BillOut, "Format6");
            BillOut.newLine();

            if (discount.trim().length() > 0 && !"0.00".equalsIgnoreCase(discount))
            {
                objPrintingUtility.funWriteTotal("DISCOUNT  " + discPer + "%", discount, BillOut, "Format6");
                BillOut.newLine();
            }
            while (rsTax.next())
            {
                objPrintingUtility.funWriteTotal(rsTax.getString(1), rsTax.getString(2), BillOut, "Format6");
                BillOut.newLine();
            }
            if (deliveryCharge != null && deliveryCharge.trim().length() > 0 && !"0.00".equalsIgnoreCase(deliveryCharge))
            {
                objPrintingUtility.funWriteTotal("DELV. CHARGE", deliveryCharge, BillOut, "Format6");
                BillOut.newLine();
            }

            if (advAmount.trim().length() > 0 && !"0.00".equalsIgnoreCase(advAmount))
            {
                objPrintingUtility.funWriteTotal("ADVANCE", advAmount, BillOut, "Format6");
                BillOut.newLine();
            }

            BillOut.write(dottedLine);
            BillOut.newLine();
            objPrintingUtility.funWriteTotal("TOTAL(ROUNDED)", grandTotal, BillOut, "Format6");
            BillOut.newLine();
            BillOut.write(dottedLine);
            BillOut.newLine();
            ...
            {
                objPrintingUtility.funWriteTotal(rs_Bill_Settlement.getString(2), rs_Bill_Settlement.getString(1), BillOut, "Format6");
                BillOut.newLine();
            }
