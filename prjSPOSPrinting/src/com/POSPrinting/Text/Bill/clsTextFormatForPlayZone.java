 ...
 * @date Aug 26, 2017
 */
public class clsTextFormatForPlayZone implements clsBillGenerationFormat
{
	...
	try
	{
	    ...
	    String sqlPlayZone = "select a.strRegisterCode,a.strMemberCode,a.strMemberName,a.strBillNo "
		    + ",time(a.dteInDateTime),time(a.dteOutDateTime),TIMEDIFF(TIME(a.dteOutDateTime), TIME(a.dteInDateTime))duration "
		    + "from tblregisterinoutplayzone a "
		    + "where a.strBillNo='" + billNo + "' ";
	    ResultSet rsmemberDtl = clsGlobalVarClass.dbMysql.executeResultSet(sqlPlayZone);
	    if (rsmemberDtl.next())
	    {
		memberCode = rsmemberDtl.getString(2);
		memberName = rsmemberDtl.getString(3);

		Time inDateTime = rsmemberDtl.getTime(5);
		String inTimeFormat = timeFormat.format(inDateTime);

		Time outDateTime = rsmemberDtl.getTime(6);
		String outTimeFormat = timeFormat.format(outDateTime);

		Time durationDateTime = rsmemberDtl.getTime(7);

		String strDuration = String.valueOf(durationDateTime);

		String durationHRS = strDuration.split(":")[0];
		String durationMNTS = strDuration.split(":")[1];

		BillOut.write("  In :" + inTimeFormat + "  Out :" + outTimeFormat);
		BillOut.newLine();
		BillOut.write("  Duration :" + durationHRS + ":" + durationMNTS + " hour");
	    }
	    rsmemberDtl.close();
	    ...
	    String SQL_BillDtl = "select sum(a.dblQuantity),left(a.strItemName,22) as ItemLine1"
		    ...
		    + " ,sum(a.dblAmount),a.strItemCode,a.strKOTNo,c.strHSNNo "
		...
		if (saleQty >= 0)
		{
			...
			String hsnNo = rs_BillDtl.getString(7).trim();
			if (hsnNo.length() > 0)
			{
			    hsnNo = "(HSN)" + hsnNo;
			    //print  HSN No.
			    ...
			    BillOut.write(" ");
			    objPrintingUtility.funPrintContentWithSpace("Left", hsnNo, 22, BillOut);//Item HSN No.                    
			    BillOut.newLine();
			}
...
//                    BillOut.newLine();
	    ...
	    BillOut.newLine();
	    BillOut.write("  Member Code :" + memberCode);
	    BillOut.newLine();
	    BillOut.write("  Member Name :" + memberName);
