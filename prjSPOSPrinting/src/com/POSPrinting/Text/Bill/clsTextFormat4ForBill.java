 ...
 * @date Aug 26, 2017
 */
public class clsTextFormat4ForBill implements clsBillGenerationFormat
{
    ...
    public void funGenerateBill(String billNo, String reprint, String formName, String transType, String billDate, String posCode, String viewORprint)
    {
        try
        {
            ...
            while (rs_BillDtl.next())
            {
                ...
                objPrintingUtility.funPrintItemDetails(rs_BillDtl.getString(5), rs_BillDtl.getString(1), rs_BillDtl.getString(6), String.valueOf(amount), BillOut, "Format4", flag_DirectBillerBlill);
            ...
            if (clsGlobalVarClass.gPointsOnBillPrint)
            {
                ...
                if (rsCRMPoints.next())
                {
                    objPrintingUtility.funWriteTotal("POINTS ", rsCRMPoints.getString(1), BillOut, "Format4");
                }
                rsCRMPoints.close();
                BillOut.newLine();
            }
            ...
            objPrintingUtility.funWriteTotal("SUB TOTAL", subTotal, BillOut, "Format4");
            BillOut.newLine();
            ...
            while (rsTax.next())
            {
                objPrintingUtility.funWriteTotal(rsTax.getString(1), rsTax.getString(2), BillOut, "Format4");
                BillOut.newLine();
            }
            if (deliveryCharge != null && deliveryCharge.trim().length() > 0 && !"0.00".equalsIgnoreCase(deliveryCharge))
            {
                objPrintingUtility.funWriteTotal("DELV. CHARGE", deliveryCharge, BillOut, "Format4");
                BillOut.newLine();
            }

            if (advAmount.trim().length() > 0 && !"0.00".equalsIgnoreCase(advAmount))
            {
                objPrintingUtility.funWriteTotal("ADVANCE", advAmount, BillOut, "Format4");
                BillOut.newLine();
            }

            BillOut.write(dottedLine);
            BillOut.newLine();
            objPrintingUtility.funWriteTotal("TOTAL(ROUNDED)", grandTotal, BillOut, "Format4");
            BillOut.newLine();
            BillOut.write(dottedLine);
            BillOut.newLine();
            ...
            {
                objPrintingUtility.funWriteTotal(rs_Bill_Settlement.getString(2), rs_Bill_Settlement.getString(1), BillOut, "Format4");
                BillOut.newLine();
            }
