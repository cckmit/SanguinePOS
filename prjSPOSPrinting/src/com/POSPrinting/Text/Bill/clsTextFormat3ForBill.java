 ...
 * @date Aug 26, 2017
 */
public class clsTextFormat3ForBill implements clsBillGenerationFormat
{
    ...
    public void funGenerateBill(String billNo, String reprint, String formName, String transType, String billDate, String posCode, String viewORprint)
    {
        try
        {
            ...
            String sql_advOrder_SuTtotal = "select a.strAdvBookingNo,a.strMessage,a.strShape,a.strNote"
                    + " from tbladvbookbillhd a, " + billhd + " b where a.strAdvBookingNo=b.strAdvBookingNo and b.strBillNo=? and date(b.dteBillDate)=? ";
            ...
            String SQL_BillDtl = "select SUM(dblQuantity),strItemName,SUM(dblAmount),strItemCode from " + billdtl + " "
                    + " where strBillNo=? and tdhYN='N' and date(dteBillDate)=? group by strItemCode ;";
            ...
            while (rs_BillDtl.next())
            {
                ...
                objPrintingUtility.funWriteToText(BillOut, rs_BillDtl.getString(1), rs_BillDtl.getString(2).toUpperCase(), String.valueOf(amount), "Format3");
                // objPrintingUtility.funWriteToText(BillOut, rs_BillDtl.getString(1), rs_BillDtl.getString(2).toUpperCase(), rs_BillDtl.getString(3),"Format3");
                BillOut.newLine();
                ...
                if (cntRecord > 0)
                {
                    sqlModifier = "select strModifierName,dblQuantity,dblAmount "
                            + "from " + billModifierdtl + " where strBillNo=? and left(strItemCode,7)=? and date(b.dteBillDate)=? ";
                    if (!clsGlobalVarClass.gPrintZeroAmtModifierOnBill)
                    {
                        sqlModifier += " and  dblAmount !=0.00 ";
                    }
                    ...
                    {
                        objPrintingUtility.funWriteToText(BillOut, "", rs_modifierRecord.getString(1).toUpperCase(), rs_modifierRecord.getString(3), "Format3");
                        BillOut.newLine();
                    }
            ...
            if (clsGlobalVarClass.gPointsOnBillPrint)
            {
                ...
                if (rsCRMPoints.next())
                {
                    objPrintingUtility.funWriteTotal("POINTS ", rsCRMPoints.getString(1), BillOut, "Format3");
                }
                rsCRMPoints.close();
                BillOut.newLine();
            }
            ...
            objPrintingUtility.funWriteTotal("SUB TOTAL", subTotal, BillOut, "Format3");
            BillOut.newLine();

            if (discount.trim().length() > 0 && !"0.00".equalsIgnoreCase(discount))
            {
                objPrintingUtility.funWriteTotal("DISCOUNT  " + discPer + "%", discount, BillOut, "Format3");
                BillOut.newLine();
            }
            /*
            ...
            while (rsTax.next())
            {
                objPrintingUtility.funWriteTotal(rsTax.getString(1), rsTax.getString(2), BillOut, "Format3");
                BillOut.newLine();
                flgForwardTax = true;
            }
            if (deliveryCharge != null && deliveryCharge.trim().length() > 0 && !"0.00".equalsIgnoreCase(deliveryCharge))
            {
                objPrintingUtility.funWriteTotal("DELV. CHARGE", deliveryCharge, BillOut, "Format3");
                BillOut.newLine();
            }

            if (advAmount.trim().length() > 0 && !"0.00".equalsIgnoreCase(advAmount))
            {
                objPrintingUtility.funWriteTotal("ADVANCE", advAmount, BillOut, "Format3");
                BillOut.newLine();
            }

            BillOut.write(dottedLine);
            BillOut.newLine();
            objPrintingUtility.funWriteTotal("TOTAL(ROUNDED)", grandTotal, BillOut, "Format3");
            BillOut.newLine();
            BillOut.write(dottedLine);
            BillOut.newLine();
            ...
            String SQL_Settlement_Breakup = "select a.dblSettlementAmt ,b.strSettelmentDesc from " + billSettlementdtl + " a ,"
                    + "tblsettelmenthd b where a.strBillNo=? and a.strSettlementCode=b.strSettelmentCode and date(a.dteBillDate)=?";
            ...
            {
                objPrintingUtility.funWriteTotal(rs_Bill_Settlement.getString(2), rs_Bill_Settlement.getString(1), BillOut, "Format3");
